...
router.post('/posts/:postId/rate', async (ctx) => {
  try {
    ...
    if (ex instanceof bouncer.ValidationError) { ctx.throw(ex.message, 400) }
    throw ex
  }
...
router.post('/me/logout', async (ctx) => {
  if (ctx.currUser) { await db.logoutSession(ctx.currUser.id, ctx.cookies.get('sessionId')) }
...
router.get('/', async (ctx) => {
  ...
  childForums.forEach((childForum) => {
    ...
    if (_.isArray(topLevelForums[parentIdx].forums)) { topLevelForums[parentIdx].forums.push(childForum) } else { topLevelForums[parentIdx].forums = [childForum] }
  })
...
router.get('/posts/:postId', async (ctx) => {
  ...
  var redirectUrl
  if (post.idx < config.POSTS_PER_PAGE) { redirectUrl = post.topic.url + '/' + post.type + '#post-' + post.id } else {
...
router.get('/pms/:id', async (ctx) => {
  ...
  var redirectUrl
  if (pm.idx < config.POSTS_PER_PAGE) { redirectUrl = pm.convo.url + '#post-' + pm.id } else {
...
router.get('/topics/:slug/:postType/first-unread', async (ctx) => {
  ...
  if (postId) { ctx.redirect('/posts/' + postId) } else { ctx.redirect(topic.url + '/' + ctx.params.postType) }
})
...
router.get('/topics/:slug/:postType', async (ctx) => {
  ...
  if (!topic.is_roleplay) { ctx.assert(!['ic', 'char'].includes(ctx.params.postType), 404) }
...
router.get('/topics/:slug', async (ctx) => {
  ...
  if (topic.forum.is_roleplay) {
    if (topic.ic_posts_count > 0) { ctx.response.redirect(ctx.request.path + '/ic') } else { ctx.response.redirect(ctx.request.path + '/ooc') }
  } else { ctx.response.redirect(ctx.request.path + '/ooc') }
})
