...
router.post('/convos', async ctx => {
    ...
    if (users.length !== unames.length) {
        ...
        ctx.flash = {
            message: [
                'danger',
                'No users were found with these names: ' +
                    rejectedUnames.join(', '),
            ],
        }
...
router.post('/convos/:convoId/pms', async ctx => {
    ...
    var toUserIds = (await db.convos.findParticipantIds(
        ctx.params.convoId
...
router.get('/convos/:convoId', async ctx => {
    ...
    if (page > totalPages) {
        var redirectUrl =
            page === 1 ? ctx.path : ctx.path + '?page=' + totalPages
...
function showConvosHandler(folder) {
    return async function _showConvosHandler(ctx) {
        ...
        var nextBeforeId =
            convos.length > 0 ? _.last(convos).latest_pm_id : null
